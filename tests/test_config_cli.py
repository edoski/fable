...
from tests.support import isolate_conf_root
...
def test_config_list_and_show_commands(tmp_path, monkeypatch) -> None:
    isolate_conf_root(tmp_path, monkeypatch)
...
def test_config_create_update_and_unset_commands(tmp_path, monkeypatch) -> None:
    conf_root = isolate_conf_root(tmp_path, monkeypatch)
...
def test_config_create_validates_cross_references(tmp_path, monkeypatch) -> None:
    isolate_conf_root(tmp_path, monkeypatch)
...
def test_config_delete_blocks_on_dependents_and_force_bypasses(tmp_path, monkeypatch) -> None:
    conf_root = isolate_conf_root(tmp_path, monkeypatch)
...
def test_config_cli_created_specs_resolve_in_runtime_commands(tmp_path, monkeypatch) -> None:
    isolate_conf_root(tmp_path, monkeypatch)
    ...
    monkeypatch.setattr("spice.cli._run_acquire", _capture_acquire)
    monkeypatch.setattr("spice.cli._run_train", _capture_train)
    monkeypatch.setattr("spice.cli._run_tune", _capture_tune)
    monkeypatch.setattr("spice.cli._run_simulate", _capture_simulate)
